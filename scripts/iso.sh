...
echo "/dev/iso9660/$DESKTOPBSD_LABEL / cd9660 ro 0 0" > $BASEDIR/etc/fstab
echo "### Running makefs to create ISO ###"
bootable="-o bootimage=i386;${BASEDIR}/boot/cdboot -o no-emul-boot"
makefs -t cd9660 $bootable -o rockridge -o label=${DESKTOPBSD_LABEL} ${ISOPATH} ${BASEDIR}
}
...
makefs -t cd9660 $bootable -o rockridge -o label=${DESKTOPBSD_LABEL} ${UEFI_ISOPATH} ${BASEDIR}
rm -f efiboot.img
echo "uefi iso done"
}
...
echo "Creating ISO..."
grub-mkrescue -o ${ISOPATH} ${BASEDIR} -- -volid ${DESKTOPBSD_LABEL}
if [ $? -ne 0 ] ; then
...
echo " making uefi iso"
bootable="-o bootimage=i386;efiboot.img -o no-emul-boot"
grub-mkrescue -o ${UEFI_ISOPATH} ${BASEDIR} -- -volid ${DESKTOPBSD_LABEL}
rm -f efiboot.img
echo "uefi iso done"
}
