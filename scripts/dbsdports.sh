...
awk '/^desktopbsd_deps/,/^"""/' ${LOCALDIR}/packages/${PACK_PROFILE} | grep -v '"""' | grep -v '#' > /tmp/${PACK_PROFILE}-gdepends

# If exist an old .packages file removes it
if [ -f /tmp/${PACK_PROFILE}-desktopbsd ] ; then
  rm -f /tmp/${PACK_PROFILE}-desktopbsd
fi
...
while read pkgs ; do
awk '/^packages/,/^"""/' ${LOCALDIR}/packages/desktopbsd.d/$pkgs  >> /tmp/${PACK_PROFILE}-gpackage
done < /tmp/${PACK_PROFILE}-gdepends

# Removes """ and # from temporary package file
cat /tmp/${PACK_PROFILE}-gpackage | grep -v '"""' | grep -v '#' > /tmp/${PACK_PROFILE}-desktopbsd
...
### Add desktopbsd ports depends to $PKGFILE
...
pkgfile="${PACK_PROFILE}-desktopbsd-deps"
